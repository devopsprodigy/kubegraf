...
export class DOPK8SDatasource {
  ...
  metricFindQuery(query) {
    ...
    switch (queryData[0]) {
      ...
      case 'node':
        return this.getNodesSingletone().then((nodes) => {
      ...
      case 'namespace':
        return this.getNamespaces().then(async (namespaces) => {
          switch (queryData[1]) {
            case 'deployment':
              const deployments = await this.getDeployments();
              const deploymentsNamespace = deployments.map((deployment) => deployment.metadata.namespace);
              return namespaces
                .filter((ns) => deploymentsNamespace.includes(ns.metadata.name))
                .map((ns) => {
                  return {
            ...
            case 'statefulset':
              ...
              return namespaces
                .filter((ns) => statefulsetsNamespace.includes(ns.metadata.name))
                .map((ns) => {
                  return {
                    text: ns.metadata.name,
                    value: ns.metadata.name,
            ...
            case 'daemonset':
              ...
              return namespaces
                .filter((ns) => daemonsetsNamespace.includes(ns.metadata.name))
                .map((ns) => {
                  return {
                    text: ns.metadata.name,
                    value: ns.metadata.name,
      ...
      case 'pod':
        return this.getPods(queryData[1]).then((pods) =>
          pods.map((pod) => {
            return {
      ...
      case 'deployment':
        return this.getDeploymentsSingletone().then((deployments) =>
          deployments
      ...
      case 'daemonset':
        return this.getDaemonsetsSingletone().then((daemonsets) =>
          daemonsets
      ...
      case 'statefulset':
        return this.getStateFulSetsSingletone().then((statefulsets) =>
          statefulsets
  ...
  __parseContainers(items, queryData) {
    ...
    if (_item.spec.template) {
      containers = _item.spec.template.spec.containers.map((cont) => cont.name);
    } else if (_item.spec.containers) {
      containers = _item.spec.containers.map((cont) => cont.name);
    }
  ...
  getNamespaces() {
    return this.__get('/api/v1/namespaces').then((result) => {
      if (!result.items) {
  ...
  getDeployments(namespace = null) {
    return this.__get('/apis/apps/v1/' + this.__addNamespace(namespace) + 'deployments').then((result) => {
      if (!result.items) {
  ...
  getStatefulsets(namespace = null) {
    return this.__get('/apis/apps/v1/' + this.__addNamespace(namespace) + 'statefulsets').then((result) => {
      if (!result.items) {
  ...
  getDaemonsets(namespace = null) {
    return this.__get('/apis/apps/v1/' + this.__addNamespace(namespace) + 'daemonsets').then((result) => {
      if (!result.items) {
  ...
  getPods(namespace) {
    return this.__get('/api/v1/' + this.__addNamespace(namespace) + 'pods').then((result) => {
      if (!result.items) {
  ...
  getServices(namespace) {
    return this.__get('/api/v1/' + this.__addNamespace(namespace) + 'services').then((result) => {
      if (!result.items) {
  ...
  getComponents() {
    return this.__get('/api/v1/componentstatuses').then((result) => {
      if (!result.items) {
  ...
  getNodesSingletone() {
    if (!this.nodesPromise) {
      this.nodesPromise = this.__get('/api/v1/nodes').then((result) => {
        if (!result.items) {
  ...
  getDeploymentsSingletone(namespace = null) {
    if (!this.deploymentsPromise) {
      this.deploymentsPromise = this.__get('/apis/apps/v1/' + this.__addNamespace(namespace) + 'deployments').then(
        (result) => {
          if (!result.items) {
  ...
  getDaemonsetsSingletone(namespace = null) {
    if (!this.daemonsetsPromise) {
      this.daemonsetsPromise = this.__get('/apis/apps/v1/' + this.__addNamespace(namespace) + 'daemonsets').then(
        (result) => {
          if (!result.items) {
  ...
  getStateFulSetsSingletone(namespace = null) {
    if (!this.statefulsetsPromise) {
      this.statefulsetsPromise = this.__get('/apis/apps/v1/' + this.__addNamespace(namespace) + 'statefulsets').then(
        (result) => {
          if (!result.items) {
  ...
  getPodsSingleton(namespace = null) {
    if (!this.podsPromise) {
      this.podsPromise = this.__get('/api/v1/' + this.__addNamespace(namespace) + 'pods').then((result) => {
        if (!result.items) {
  ...
  getNodes() {
    return this.__get('/api/v1/nodes').then((result) => {
      if (!result.items) {
  ...
  getJobs() {
    return this.__get('/apis/batch/v1/jobs').then((result) => {
      if (!result.items) {
  ...
  getCronJobs() {
    return this.__get('/apis/batch/v1beta1/cronjobs').then((result) => {
      if (!result.items) {
  ...
  getEvents() {
    return this.__get('/api/v1/events').then((result) => {
